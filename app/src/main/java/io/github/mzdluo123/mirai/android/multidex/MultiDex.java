 ...
 */
package io.github.mzdluo123.mirai.android.multidex;
...
import io.github.mzdluo123.mirai.android.miraiconsole.DexPluginClassLoader;
...
public final class MultiDex {
    static final String TAG = "MA MultiDex";
     ...
     *         extension.
     */
    public static void install(Context context, DexPluginClassLoader loader, File installFile) {
        Log.i(TAG, "install");
        if (IS_VM_MULTIDEX_CAPABLE) {
            Log.i(TAG, "VM has multidex support, MultiDex support library is disabled.");
            try {
                clearOldDexDir(context, installFile);
            } catch (Throwable t) {
        ...
        try {
            synchronized (installedApk) {
                String apkPath = installFile.getAbsolutePath();
                ...
                File dexDir = new File(context.getFilesDir() + File.separator + installFile.getName(), SECONDARY_FOLDER_NAME);
                List<File> files = MultiDexExtractor.load(context, installFile, dexDir, false);
                if (checkValidZipFiles(files)) {
                    installSecondaryDexes(loader, dexDir, files);
                } else {
                    Log.w(TAG, "Files were not valid zip files.  Forcing a reload.");
                    // Try again, but this time force a reload of the zip file.
                    files = MultiDexExtractor.load(context, installFile, dexDir, true);
                    if (checkValidZipFiles(files)) {
                        installSecondaryDexes(loader, dexDir, files);
                    } else {
    ...
    private static void clearOldDexDir(Context context, File installFile) throws Exception {
        synchronized (installedApk) {
            String apkPath = installFile.getAbsolutePath();
            if (installedApk.contains(apkPath)) {
                return;
